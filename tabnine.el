...
(defconst tabnine--method-autocomplete "Autocomplete")
(defconst tabnine--method-prefetch "Prefetch")
(defconst tabnine--method-getidentifierregex "GetIdentifierRegex")
(defconst tabnine--message-buffer-name "*TabNineMessages*")
...
(defcustom tabnine-log-level "Debug"
...
(defcustom tabnine-log-messages nil
...
(defcustom tabnine-file-type-regex
  ".*rs$\\|.*java$\\|.*go$\\|.*c$\\|.*h$\\|.*cpp$\\|.*py$\\|.*js$\\|.*ts$\\|.*css$\\|.*html$\\|.*htm$\\|.*tf$"
  "Regular expression for file extensions to pre-register."
  :group 'tabnine
  :type 'string)
...
;; TODO: Complete implimentation, call tabnine with the files
;; so they can be pre-analysed

(defun tabnine-preload-source ()
  "Preload source files."
  (interactive)
  (let ((filelist
         (directory-files-recursively
          (projectile-project-root)
          tabnine-file-type-regex)
         ))
    (cl-loop for thefile in filelist
             do (progn
                  (let ((preload-request (list
                    ...
                    (tabnine-send-request preload-request)
                    )
                  (message thefile)
                  )
             )
...
(defun tabnine-accept-completion ()
  ...
  (let ((old-prefix-length (length (cdr (assoc 'old_prefix tabnine--response))))
        (old-comp-suffix-lenght (length (cdr (assoc 'old_suffix tabnine--current-result))))
  ...
  (when (s-ends-with-p "\n" output)
    (let ((response
           (mapconcat #'identity
                      ...
                      nil)))
      (setq tabnine--response
            (tabnine--decode response)
            tabnine--response-chunks nil))))

(defun tabnine--decode (msg)
  "Decode TabNine server response MSG, and return the decoded object."
  (when tabnine-log-messages
    (with-current-buffer (get-buffer-create tabnine--message-buffer-name)
      (goto-char (point-max))
      (insert msg)))
  (ignore-errors
